...
```
app push → wiki-processor:
  Phase 1（無鎖並發）: 讀 wiki + LLM 萃取 + 〔embed 該 app 的 entries〕
  Phase 2（CAS loop）: merge + 條件寫入 wiki.json   ← 事實來源在這裡定案
  之後（best-effort）: PG replace_app_entries（單一交易）
                       → POST mcp-server /cache/invalidate
mcp-server 讀取:
  PG 可用     → 索引查詢（語意 / trigram 關鍵字 / 點查）
  PG 不可用   → 原本的 cached-wiki 路徑（自動 fallback，行為與今日相同）
```
