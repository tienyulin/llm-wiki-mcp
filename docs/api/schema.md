...
## 原生 MCP 端點（2026-06-19）

mcp-server 同時掛載原生 [MCP](https://modelcontextprotocol.io) 伺服器（Streamable
HTTP transport，stateless），端點 `POST /mcp/`。Claude / agent 可直接連線，不需自寫
REST client。工具是既有 `QueryService` 的薄包裝：`search_apis`、`semantic_search`、
`list_apis`、`get_api_detail`、`list_concepts`、`get_concept`、`get_overview`、`wiki_info`。

加入 Claude Code：`claude mcp add --transport http llm-wiki http://localhost:8002/mcp/`

正式環境設 `MCP_ALLOWED_HOSTS`（逗號分隔）開啟 DNS-rebinding 保護；未設則關閉（dev 預設）。
設計緣由見 mcp-server 的 `docs/architecture/mcp-transport.md`。
